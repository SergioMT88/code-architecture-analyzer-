...
AMBIGUOUS_REPLACEMENTS = {"l": "ln", "I": "idx", "O": "result"}
...
class RefactoringOrchestrator:
    ...
    def _rename_ambiguous_vars(self, code: str, changes: List[Dict[str, Any]]) -> str:
        ...
        # Collect (lineno, col_offset, old, new) for the target itself and any
        # ast.Name references whose .id matches, within each comprehension scope.
            ...
            if isinstance(node, (ast.ListComp, ast.SetComp, ast.DictComp, ast.GeneratorExp)):
                for gen in node.generators:
                    ...
                    if isinstance(target, ast.Name) and target.id in AMBIGUOUS_NAMES:
                        old, new = target.id, AMBIGUOUS_REPLACEMENTS[target.id]
                        substitutions.append((target.lineno, target.col_offset, old, new))
                        for sub in ast.walk(node):
                            if isinstance(sub, ast.Name) and sub.id == old and sub is not target:
                                substitutions.append((sub.lineno, sub.col_offset, old, new))
                        reported.append((target.lineno, old, new))

        if not substitutions:
            return code

        lines = code.split("\n")
        substitutions.sort(key=lambda s: (s[0], -s[1]))
        for lineno, col, old, new in substitutions:
            if not 1 <= lineno <= len(lines):
                continue
            line = lines[lineno - 1]
            end = col + len(old)
            if end > len(line) or line[col:end] != old:
                continue
            before_ch = line[col - 1] if col > 0 else ""
            after_ch = line[end] if end < len(line) else ""
            if before_ch.isalnum() or before_ch == "_":
                continue
            if after_ch.isalnum() or after_ch == "_":
                continue
            lines[lineno - 1] = line[:col] + new + line[end:]

        new_code = "\n".join(lines)

        for lineno, old, new in reported:
            reason = (
                "confundido com digito '1'" if old in ("l", "I")
                else "confundido com digito '0'"
            )
            changes.append({
                "type": "ambiguous_variable",
                "description": (
                    f"variavel ambigua '{old}' renomeada para '{new}' na linha {lineno} "
                    f"({reason}; PEP 8 E741)"
                ),
    ...
    def phase3_tests(self) -> Dict[str, Any]:
        ...
        header = f'"""Tests for {self.filepath.name} - generated by Code Architecture Analyzer v{__version__}"""'
