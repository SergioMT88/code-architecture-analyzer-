...
from code_analyzer.analyzer.semantic import compare_directory, compare_files
from code_analyzer.config import DEFAULT_CONFIG, load_config
from code_analyzer.constants import DEFAULT_MIN_SCORE
...
def _print_usage(json_mode: bool = False) -> None:
    ...
    if json_mode:
        ...
        print("  agent    <arquivo.py>        Gera prompt para agente de IA")
...
def _smart_config(project_type: str) -> dict:
    ...
    cfg["min_score"] = DEFAULT_MIN_SCORE
    return cfg
...
def _handle_init(json_mode: bool = False) -> int:
    ...
    if agents_created:
        agents_path.write_text(
            generate_agents_md(project_type, version), encoding="utf-8"
        )
    ...
    type_label = {"django": "Django", "flask": "Flask", "fastapi": "FastAPI"}.get(
        project_type, "Python generico"
    )
    print("\n  Code Architecture Analyzer — Configuracao Inteligente")
    print(f"\n  Projeto detectado: {type_label}\n")

    if config_created:
        print("  Criado:   .analyzer.json")
        print(
            f"            architecture_style: {project_type} | min_score: {min_score}"
        )
    else:
        print("  Mantido:  .analyzer.json  (ja existia)")

    if agents_created:
        print("\n  Criado:   AGENTS.md")
        print("            Regras do projeto validaveis por code-analyze check")
        print("            Edite a secao ## [rules] com as regras do seu projeto")
    else:
    ...
    if precommit_created:
        ...
        print(
            f"            Hook: code-analyze --min-score {min_score} --no-refactor --quiet"
        )
    else:
        print("\n  Mantido:  .pre-commit-config.yaml  (ja existia)")

    print("\n  Proximos passos:")
    print("    1. Edite AGENTS.md — adicione as regras especificas do seu projeto")
    print("    2. pip install pre-commit && pre-commit install")
    print("    Pronto — cada commit valida codigo e regras automaticamente.\n")
    return 0
...
def _run_history(argv: list) -> int:
    ...
    print(
        "  "
        + f"{'Execução (Data/Hora)':<20} | {'MI':<4} | {'Grade':<5} | {'Critérios com problemas (Score < 10)'}"
    )
    print("  " + "-" * 95)
...
def _run_duplication_check(argv: list) -> int:
    ...
    if len(files) < 2:
        _emit(
            {
                "success": False,
                "error": "Especifique 2 arquivos: code-analyze dup a.py b.py",
            },
            json_mode,
        )
        return 1
    ...
    for i, d in enumerate(duplicates, 1):
        ...
        print(f"  {i}. {names}")
        print("     Sugestao: consolide em uma unica funcao parametrizavel.\n")
    return 0
...
def _run_project_check(argv: list) -> int:
    ...
    if not dirpath:
        _emit(
            {
                "success": False,
                "error": "Diretorio nao especificado. Uso: code-analyze project <dir>",
            },
            json_mode,
        )
        return 1
    ...
    if not Path(dirpath).is_dir():
        _emit(
            {"success": False, "error": f"Diretorio nao encontrado: {dirpath}"},
            json_mode,
        )
        return 1
    ...
    print(
        f"  Arquivos analisados: {result['files_scanned']} | Funcoes: {result['functions_analyzed']}"
    )
    ...
    print(
        f"  Top duplicacoes (estrutura de funcao {mode_label} em arquivos diferentes):\n"
    )
    for i, dup in enumerate(duplicates[:10], 1):
        funcs = dup.get("functions", [])
        files = dup.get("files", [])
        func_names = " | ".join(
            f"\033[94m{f['name']}\033[0m ({Path(f['file']).name}:{f['lineno']})"
            for f in funcs[:4]
        )
        ...
        if len(files) > 1:
            print(
                f"     \033[90mArquivos: {', '.join(Path(fp).name for fp in files)}\033[0m"
            )
        print(
            "     \x1b[92mSugestao:\x1b[0m Consolide em uma unica funcao parametrizavel.\n"
        )

    return 0


def _parse_agent_args(argv: list) -> tuple:
    filepath = None
    output_file = None
    pipe_tool = None
    auto_mode = False
    ...
    while i < len(argv):
        if argv[i] == "--output" and i + 1 < len(argv):
            ...
            i += 2
        elif argv[i] == "--pipe" and i + 1 < len(argv):
            pipe_tool = argv[i + 1]
            i += 2
        elif argv[i] == "--auto":
            auto_mode = True
            ...
            i += 1
    return filepath, output_file, pipe_tool, auto_mode


def _handle_agent_output(prompt: str, json_mode: bool, output_file: str = None, pipe_tool: str = None, auto_mode: bool = False, filepath: str = "") -> int:
    if json_mode:
        _emit({"success": True, "file": filepath, "prompt": prompt}, json_mode)
        return 0
    if output_file:
        Path(output_file).write_text(prompt, encoding="utf-8")
        print(f"✅ Prompt salvo em: {output_file}")
        return 0
    if auto_mode or pipe_tool:
        available_tools = _detect_ai_tools()
        if auto_mode:
            if not available_tools:
                print("⚠️  Nenhuma ferramenta de IA detectada.")
                print("   Instale uma das opções:")
                print("   - claude (Claude Code)")
                print("   - ollama (Ollama local)")
                print("   - python openai (OpenAI API)")
                print("\n   Ou use --output <file> para salvar o prompt.")
                print("\n   Prompt gerado (salvando em /tmp/agent_prompt.md)...")
                temp_file = Path("/tmp/agent_prompt.md")
                temp_file.write_text(prompt, encoding="utf-8")
                print(f"   ✅ Salvo em: {temp_file}")
                print(f"\n   Use manualmente: cat {temp_file} | <sua_ferramenta>")
                return 0
            pipe_tool = available_tools[0]
            print(f"🔍 Ferramenta detectada: {pipe_tool}")
        result = _pipe_to_agent(prompt, pipe_tool)
        if result == 0:
            print(f"\n✅ Prompt enviado para {pipe_tool}")
        else:
            print(f"\n❌ Erro ao enviar para {pipe_tool}")
            print("   Tentando salvar em arquivo...")
            temp_file = Path(f"/tmp/agent_prompt_{Path(filepath).stem}.md")
            temp_file.write_text(prompt, encoding="utf-8")
            print(f"   ✅ Salvo em: {temp_file}")
        return result
    print(prompt)
...
def _run_agent_review(argv: list, json_mode: bool) -> int:
    filepath, output_file, pipe_tool, auto_mode = _parse_agent_args(argv)
    if not filepath:
        _emit({"success": False, "error": "Arquivo nao especificado"}, json_mode)
        return 1
    if not Path(filepath).exists():
        _emit({"success": False, "error": f"Arquivo nao encontrado: {filepath}"}, json_mode)
        return 1
    config = load_config(filepath, quiet=True)
    source_code = Path(filepath).read_text(encoding="utf-8", errors="ignore")
    analysis_result = run_analysis(filepath, config)
    ...
    return _handle_agent_output(prompt, json_mode, output_file, pipe_tool, auto_mode, filepath)


def _detect_ai_tools() -> list:
    """Detect available AI tools on the system."""
...
def _pipe_to_agent(prompt: str, tool: str) -> int:
    """Pipe the prompt to a specific AI tool."""
    try:
        if tool == "claude":
            ...
            # Fallback: save to file and show instructions
            temp_file = Path("/tmp/agent_prompt.md")
            temp_file.write_text(prompt, encoding="utf-8")
            print(f"📄 Prompt salvo em: {temp_file}")
            print("\nPara usar com sua ferramenta de IA:")
            print(f"  cat {temp_file} | <sua_ferramenta>")
            return 0

        else:
            print(f"Ferramenta não suportada: {tool}")
            return 1

    except subprocess.TimeoutExpired:
        print("Timeout: ferramenta demorou mais de 5 minutos")
        return 1
