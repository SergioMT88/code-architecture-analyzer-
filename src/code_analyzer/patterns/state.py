...
def detect(tree: ast.AST, code: str) -> Optional[PatternDetection]:
        ...
        for node in ast.walk(tree):
            ...
            is_state = False
            if any(k in class_name for k in ("state", "status", "phase")):
                is_state = True
                confidence = 0.8
            if method_names.intersection({"transition", "next_state", "set_state"}):
                is_state = True
                confidence = 0.75
            ...
            if is_state:
                ...
                checks.append(PatternCheck(
                    name="Transitions are explicit",
                    status="OK",
                    description="State should know which state to transition to next",
                ))

                checks.append(PatternCheck(
                    name="No switch in context",
                    status="OK",
                    description="Context should delegate to current state, not use switch/if",
                ))
